...
router.get('/', requireAuth, async (req, res: Response) => {
  ...
  const rows = (data ?? []).map(d => ({
    ...d,
    code: assetMap[d.asset_id]?.code ?? '',
    name: assetMap[d.asset_id]?.name ?? '',
  }))
...
router.get('/summary', requireAuth, async (req, res: Response) => {
  ...
  let q = supabaseAdmin
    .from('dividends')
    .select('asset_id, ex_date, amount_brl, amount_total, currency, dividend_type')
    .in('asset_id', assetIds)
    .eq('user_id', userId)
  ...
  for (const r of rows) {
    if (!byAssetMap[r.asset_id]) {
      byAssetMap[r.asset_id] = {
        code: assetMap[r.asset_id]?.code ?? '',
        name: assetMap[r.asset_id]?.name ?? '',
        total_brl: 0, count: 0,
      }
    }
