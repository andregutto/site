...
// Returns the financial month key (YYYY-MM) for a transaction date given a cycle start day.
// Example: cycleDay=27, date='2026-05-28' → '2026-06' (counted in June's budget)
...
// Returns the actual date range [start, end] for a financial month key.
// Example: cycleDay=27, yearMonth='2026-06' → { start:'2026-05-27', end:'2026-06-26' }
...
router.post('/accounts', requireAuth, async (req, res: Response) => {
  ...
  if (create_asset) {
    ...
    // Find or create the Caixa asset class
...
// GET /api/finances/categories/monthly-history — monthly totals per category for a date range
...
router.post('/moments', requireAuth, async (req, res: Response) => {
  const { userId } = req as AuthRequest
  const { name, description, icon, color, start_date, end_date, cover_image_url, cover_image_position, budget } = req.body
  const { data, error } = await supabaseAdmin
    .from('finance_moments')
    .insert({ user_id: userId, name, description, icon: icon ?? '✨', color: color ?? '#7C3AED', start_date: start_date ?? null, end_date: end_date ?? null, cover_image_url: cover_image_url ?? null, cover_image_position: cover_image_position ?? '50% 50%', budget: budget ?? null })
    .select().single()
  if (error) { res.status(500).json({ error: error.message }); return }
  res.json(data)
})
