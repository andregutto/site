// BCB — séries históricas do Banco Central do Brasil
// Série 12:   CDI diário (% ao dia)
// Série 433:  IPCA mensal (%)
// Série 1178: Selic diária (% ao dia)
...
export interface BCBRate {
  date: Date
  value: number   // % (ex: 0.0522 para CDI diário, 0.16 para IPCA mensal)
}
...
function parseDate(str: string): Date {
  // Formato BCB: "DD/MM/YYYY" ou "MM/YYYY" (IPCA)
  const parts = str.split('/')
  if (parts.length === 2) {
    // IPCA: MM/YYYY → primeiro dia do mês
...
export async function getIPCARates(start: Date, end: Date): Promise<BCBRate[]> {
  try {
    ...
    // Fallback: BrasilAPI /taxas/v1 — retorna apenas taxa anual atual, sem histórico.
    // Quando BCB está indisponível, injeta um único ponto mensal aproximado.
    ...
    // Generate one synthetic monthly rate per calendar month in the range
