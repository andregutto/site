...
async function computeFriendsAndGroups(userId: string): Promise<{ friends: HomeFriendEntry[]; groups: HomeGroupEntry[]; moments: HomeMomentEntry[]; total_balance: { currency: string; amount: number }[] }> {
  const [
    ...
    { data: ownedMoments }, { data: memberMomentRows },
  ] = await Promise.all([
  ...
  const [{ data: memberGroupsRaw }, { data: extraMoments }] = await Promise.all([
  ...
  // As três buscas abaixo não dependem uma da outra (despesas de Momento,
  // transações em categoria compartilhada, e membros dos grupos pros
  // avatares) — rodam em paralelo em vez de em sequência.
  const [expensesResult, sharedCatsResult, groupMembersResult] = await Promise.all([
    allMomentIds.length
      ? supabaseAdmin
    ...
    groupMap.size > 0
      ? (async () => {
          ...
          if (catIds.length === 0) return { catGroupMap, txns: [] as any[] }
          const { data: txns } = await supabaseAdmin
            ...
            .limit(500)
          return { catGroupMap, txns: txns ?? [] }
        })()
      : Promise.resolve({ catGroupMap: new Map<number, number>(), txns: [] as any[] }),
    groupMap.size > 0
      ? supabaseAdmin.from('shared_group_members').select('group_id, user_id').in('group_id', [...groupMap.keys()]).eq('status', 'active').not('user_id', 'is', null)
      : Promise.resolve({ data: [] as any[] }),
  ])
  ...
  for (const t of sharedCatsResult.txns ?? []) {
    const gid = sharedCatsResult.catGroupMap.get((t as any).shared_category_id)
  ...
  for (const m of groupMembersResult.data ?? []) {
...
async function loadNextTrip(userId: string, todayStr: string) {
...
async function loadActiveMoment(userId: string, todayStr: string) {
  const [{ data: tripMomentRows }, { data: momentsRaw }] = await Promise.all([
    supabaseAdmin.from('voyage_trip_moments').select('moment_id').eq('user_id', userId),
    supabaseAdmin
...
async function loadCommunityUnseen(userId: string): Promise<number> {
  try {
    ...
    return replies.filter(r => !dismissed.has(r.key)).length
  } catch {
    return 0 // ponto vermelho é opcional
  }
}
...
router.get('/today', async (req: any, res: any) => {
  ...
  try {
    // Nenhuma dessas seções depende do resultado de outra — rodam todas em
    // paralelo em vez de uma cadeia sequencial de ~15-20 idas ao banco.
    const [display, hotTopics, nextTrip, activeMoment, monthSummary, communityUnseen, friendsAndGroups] = await Promise.all([
      userDisplay(userId),
      loadHotTopics(),
      loadNextTrip(userId, todayStr),
