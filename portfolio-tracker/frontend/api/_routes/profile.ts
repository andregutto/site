...
router.patch('/', requireAuth, async (req, res: Response) => {
  const { userId } = req as AuthRequest
  const { first_name, last_name, country, portfolio_start_date, allocation_targets, institution_data, avatar_url } = req.body as {
  ...
  const meta = { ...(current?.user_metadata ?? {}), ...Object.fromEntries(
    Object.entries({ first_name, last_name, country, portfolio_start_date, allocation_targets, institution_data, avatar_url })
