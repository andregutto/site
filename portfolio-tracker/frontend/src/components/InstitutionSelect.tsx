...
// Cache module-level para não buscar a cada render
let cachedList: InstitutionList | null = null

export default function InstitutionSelect({ value, onChange, placeholder = 'Banco ou corretora...' }: Props) {
  ...
  // Sincroniza query com value externo ao montar/resetar
  ...
  useEffect(() => {
    function onOutside(e: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(e.target as Node)) {
        setOpen(false)
        // Se query não corresponde a nenhuma opção, mantém o que foi digitado
  ...
  function filterGroup(items: string[]) {
    if (!q) return items.slice(0, 12)  // sem filtro: mostra top 12
  ...
  // Mostra opção "Adicionar X" se não há match exato
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && list && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-56 overflow-y-auto">
          {!list ? (
            <p className="px-3 py-3 text-xs text-gray-400">Carregando...</p>
          ) : (
            <>
              {brFiltered.length > 0 && (
                <>
