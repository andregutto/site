            ...
            <tbody className="divide-y divide-[var(--arvo-border-soft)]">
              {visible.map(p => (
                <tr key={p.ticker} className={p.active ? '' : 'opacity-50'}>
                  ...
                  <td className={`px-4 py-2 text-right tabular-nums ${p.net_qty < 0 ? 'text-red-600 font-semibold dark:text-red-300' : 'text-[var(--arvo-fg)]'}`}>
                    {new Intl.NumberFormat('pt-BR').format(p.net_qty)}
                    {p.net_qty < 0 && <span className="ml-1 text-xs font-normal text-red-500 dark:text-red-400" title="Qtd negativa pode indicar evento corporativo não registrado">⚠</span>}
                  </td>
                  <td className="px-4 py-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.active ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-[var(--arvo-track-bg)] text-[var(--arvo-fg-muted)]'}`}>
...
function StatCard({ label, value, sub, color = 'gray' }: { label: string; value: string | number; sub?: string; color?: 'gray' | 'blue' | 'amber' | 'green' | 'red' }) {
  const colors = {
    ...
    blue:  'bg-blue-50 border-blue-100 text-blue-900 dark:bg-blue-950/40 dark:border-blue-900 dark:text-blue-200',
    amber: 'bg-amber-50 border-amber-100 text-amber-900 dark:bg-amber-950/40 dark:border-amber-900 dark:text-amber-200',
    green: 'bg-green-50 border-green-100 text-green-900 dark:bg-green-950/40 dark:border-green-900 dark:text-green-200',
    red:   'bg-red-50 border-red-100 text-red-900 dark:bg-red-950/40 dark:border-red-900 dark:text-red-200',
  }
  return (
...
export default function ImportB3Page() {
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div
          ...
          className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-colors ${
            movDragOver ? 'border-[var(--arvo-fg)] bg-blue-50 dark:bg-blue-950/40' : 'border-[var(--arvo-border)] bg-[var(--arvo-surface)] hover:border-[var(--arvo-border)]'
          }`}
        >
        ...
        {movError && <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-700 text-sm dark:bg-red-950/40 dark:border-red-900 dark:text-red-300">{movError}</div>}
  ...
  if (activeTab === 'movimentacao' && movStep === 'preview' && movParseResult) {
    ...
    const eventColor = {
      bonificacao: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300',
      desdobro:    'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
      subscricao:  'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300',
    } as const
    ...
    return (
      <div className="space-y-6">
        ...
        {notFound.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 dark:bg-amber-950/40 dark:border-amber-900">
            <h2 className="font-semibold text-amber-900 text-sm mb-2 dark:text-amber-200">Ativos não encontrados no sistema ({notFound.length})</h2>
            <p className="text-xs text-amber-700 mb-3 dark:text-amber-300">Esses eventos serão ignorados. Cadastre os ativos primeiro e reimporte.</p>
            <div className="flex flex-wrap gap-2">
              {notFound.map(a => (
                <span key={a.ticker} className="text-xs px-2.5 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-800 font-semibold dark:bg-amber-900/40 dark:border-amber-800 dark:text-amber-200">
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        {movExecResult.skipped_tickers.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 dark:bg-amber-950/40 dark:border-amber-900">
            <h2 className="font-semibold text-amber-900 text-sm mb-2 dark:text-amber-200">Tickers não encontrados no sistema</h2>
            <div className="flex flex-wrap gap-2">
              {movExecResult.skipped_tickers.map(t => (
                <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-800 font-semibold dark:bg-amber-900/40 dark:border-amber-800 dark:text-amber-200">{t}</span>
              ))}
            </div>
          </div>
        )}
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div
          ...
          onDrop={onDrop}
          className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-colors ${
            dragOver ? 'border-[var(--arvo-fg)] bg-blue-50 dark:bg-blue-950/40' : 'border-[var(--arvo-border)] bg-[var(--arvo-surface)] hover:border-[var(--arvo-border)]'
          }`}
        >
          <div className="text-4xl mb-4">📊</div>
          <p className="font-semibold text-[var(--arvo-fg)] mb-1">Arraste o arquivo .xlsx aqui</p>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-700 text-sm dark:bg-red-950/40 dark:border-red-900 dark:text-red-300">{error}</div>
        )}
  ...
  if (step === 'preview' && parseResult) {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Assets to create */}
        {toCreate.length > 0 && (
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 dark:bg-blue-950/40 dark:border-blue-900">
            <h2 className="font-semibold text-blue-900 text-sm mb-3 dark:text-blue-200">
              Ativos a criar ({toCreate.length}) <span className="font-normal text-blue-600 dark:text-blue-300">— inativos se posição líquida = 0</span>
            </h2>
            <div className="flex flex-wrap gap-2">
              {toCreate.map(a => (
                <span key={a.ticker} className={`text-xs px-2.5 py-1 rounded-full font-semibold border ${
                  a.net_qty > 0
                    ? 'bg-blue-100 border-blue-200 text-blue-800 dark:bg-blue-900/40 dark:border-blue-900 dark:text-blue-200'
        ...
        {toClean.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 dark:bg-amber-950/40 dark:border-amber-900">
            <h2 className="font-semibold text-amber-900 text-sm mb-2 dark:text-amber-200">
        ...
        {found.length > 0 && (
          <div className="bg-green-50 border border-green-100 rounded-2xl p-4 dark:bg-green-950/40 dark:border-green-900">
            <h2 className="font-semibold text-green-900 text-sm mb-2 dark:text-green-200">
              Ativos já cadastrados sem aportes ({found.length})
            </h2>
            <div className="flex flex-wrap gap-2">
              {found.map(a => (
                <span key={a.ticker} className="text-xs px-2.5 py-1 rounded-full bg-green-100 border border-green-200 text-green-800 font-semibold dark:bg-green-900/40 dark:border-green-900 dark:text-green-200">
                  {a.ticker}
                </span>
              ))}
            </div>
                  ...
                  <tr key={i} className="hover:bg-[var(--arvo-surface-2)]">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        op.type === 'buy' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'
                      }`}>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 dark:bg-amber-950/40 dark:border-amber-900">
          <div>
            <p className="text-sm font-semibold text-amber-900 dark:text-amber-200">Faça um backup antes de confirmar</p>
          ...
          <button
            ...
            className="px-4 py-2 border border-amber-400 text-amber-800 bg-[var(--arvo-surface)] rounded-xl text-sm font-semibold hover:bg-amber-50 transition-colors shrink-0 dark:border-amber-700 dark:text-amber-200 dark:hover:bg-amber-950/60"
          >
            ⬇ Baixar backup CSV
          </button>
        </div>
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        {syncResult != null && (
          <div className={`border rounded-2xl p-5 space-y-3 ${
            syncResult.errors > 0 ? 'bg-amber-50 border-amber-200 dark:bg-amber-950/40 dark:border-amber-900' : 'bg-green-50 border-green-100 dark:bg-green-950/40 dark:border-green-900'
          }`}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className={`text-sm font-semibold ${syncResult.errors > 0 ? 'text-amber-900 dark:text-amber-200' : 'text-green-900 dark:text-green-200'}`}>
                  Histórico de preços sincronizado
                </p>
                <p className={`text-xs mt-0.5 ${syncResult.errors > 0 ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>
            ...
            {syncResult.errors > 0 && (() => {
              ...
              return (
                <div className="border-t border-amber-200 pt-3 space-y-3 dark:border-amber-900">
                  ...
                  {active.length > 0 && (
                    <div className="space-y-1.5">
                      <p className="text-xs font-medium text-amber-800 dark:text-amber-200">Em carteira — impacto na Performance:</p>
                      <div className="flex flex-wrap gap-2">
                        {active.map(d => (
                          <span key={d.code} className="text-xs px-2.5 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-800 font-semibold dark:bg-amber-900/40 dark:border-amber-800 dark:text-amber-200">
                            {d.code}
                          </span>
                        ))}
                      </div>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 text-xs text-blue-800 space-y-1 dark:bg-blue-950/40 dark:border-blue-900 dark:text-blue-200">
        ...
        {execResult.skipped_tickers.length > 0 && (
          <div className="bg-red-50 border border-red-100 rounded-2xl p-4 dark:bg-red-950/40 dark:border-red-900">
            <h2 className="font-semibold text-red-900 text-sm mb-2 dark:text-red-200">Tickers não importados</h2>
            <div className="flex flex-wrap gap-2">
              {execResult.skipped_tickers.map(t => (
                <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-red-100 border border-red-200 text-red-700 font-semibold dark:bg-red-900/40 dark:border-red-900 dark:text-red-300">{t}</span>
              ))}
            </div>
          </div>
        )}
