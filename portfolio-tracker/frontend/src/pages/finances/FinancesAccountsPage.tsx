...
// ── Link portfolio modal ──────────────────────────────────────────────────────

interface ManualAssetOption {
  id: number
  name: string
  currency: string
...
interface LinkPortfolioProps {
  ...
  manualAssets: ManualAssetOption[]
  onLink: (assetId: number) => Promise<void>
  onClose: () => void
  saving: boolean
}

function LinkPortfolioModal({ account, manualAssets, onLink, onClose, saving }: LinkPortfolioProps) {
...
export default function FinancesAccountsPage() {
  ...
  const [linkModal,        setLinkModal]        = useState<FinanceAccount | null>(null)
  const [manualAssets,     setManualAssets]     = useState<ManualAssetOption[]>([])
  const [linkSaving,       setLinkSaving]       = useState(false)
  const [syncingPortfolio, setSyncingPortfolio] = useState<number | null>(null)
  const [syncPortfolioOk,  setSyncPortfolioOk]  = useState<number | null>(null)
  ...
  async function openLinkModal(acc: FinanceAccount) {
    if (manualAssets.length === 0) {
      try {
        const all = await apiFetch<{ id: number; name: string; currency: string; asset_type: string; exchange: string | null }[]>('/assets')
        setManualAssets(all.filter(a => a.asset_type === 'manual').map(a => ({ id: a.id, name: a.name, currency: a.currency, exchange: a.exchange })))
      } catch { /* ignore */ }
    }
    setLinkModal(acc)
  }
  ...
  async function unlinkFromAsset(accountId: number) {
  ...
  async function syncToPortfolio(accountId: number) {
    ...
    setSyncPortfolioOk(null)
    try {
      await apiFetch(`/finances/accounts/${accountId}/sync-portfolio`, { method: 'POST' })
      setSyncPortfolioOk(accountId)
      setTimeout(() => setSyncPortfolioOk(null), 3000)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 style={{ fontFamily: "'Tenor Sans', sans-serif", fontSize: 18, letterSpacing: '0.06em', color: 'var(--arvo-black)' }}>{t.finances.accountsPageTitle}</h1>
                ...
                {acc.linked_asset_id ? (
                  <>
                    <span className="text-xs text-emerald-600 flex-1 flex items-center gap-1">
                      ...
                      Vinculado ao portfólio
                      {syncPortfolioOk === acc.id && <span className="text-emerald-500 ml-1">· Sincronizado ✓</span>}
                    </span>
                    <button
      ...
      {linkModal && (
        <LinkPortfolioModal
          ...
          onLink={(assetId) => linkToAsset(linkModal.id, assetId)}
