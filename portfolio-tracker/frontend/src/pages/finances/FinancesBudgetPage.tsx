...
export default function FinancesBudgetPage() {
  ...
  const totalBudget  = data.envelopes.reduce((s, e) => s + e.budget_amount, 0)
  const totalCatBudget = data.envelopes.reduce((s, e) => s + e.categories.reduce((cs, c) => cs + (c.budget_monthly ?? 0), 0), 0)
