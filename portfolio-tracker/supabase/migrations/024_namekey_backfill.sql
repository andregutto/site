-- Comprehensive name_key backfill for asset_classes and finance_categories
-- Covers accented/unaccented variants, singular/plural, alternate spellings

-- ── Asset classes ─────────────────────────────────────────────────────────────

UPDATE asset_classes SET name_key = 'classAcoesBrasil'
WHERE name_key IS NULL AND LOWER(unaccent(name)) IN (
...
WHERE name_key IS NULL AND LOWER(unaccent(name)) IN (
  'educacao','educação','education','education','ensino',
...
WHERE name_key IS NULL AND LOWER(unaccent(name)) = 'airbnb';
