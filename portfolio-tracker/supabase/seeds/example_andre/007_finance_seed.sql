...
INSERT INTO finance_envelopes (user_id, name, pct_target, color, type, icon, sort_order) VALUES
  (uid, 'Gastos Essenciais',     50, '#3B82F6', 'essential',  '🏠', 1),
  (uid, 'Investimentos',         30, '#10B981', 'investment', '📈', 2),
  (uid, 'Gastos Não Essenciais', 10, '#EC4899', 'free',       '🎯', 3),
  (uid, 'Torrar',                10, '#A855F7', 'free',       '🎉', 4)
ON CONFLICT DO NOTHING;

SELECT id INTO env_ess FROM finance_envelopes WHERE user_id = uid AND name = 'Gastos Essenciais';
SELECT id INTO env_fut FROM finance_envelopes WHERE user_id = uid AND name = 'Investimentos';
SELECT id INTO env_res FROM finance_envelopes WHERE user_id = uid AND name = 'Gastos Não Essenciais';
SELECT id INTO env_liv FROM finance_envelopes WHERE user_id = uid AND name = 'Torrar';
...
INSERT INTO finance_categories (user_id, envelope_id, name, color, icon, budget_monthly, keyword_rules) VALUES
  ...
  (uid, env_fut, 'Poupança',      '#10B981', '🏦',  NULL,   '["epargne","poupanca","livret","reserva"]')
ON CONFLICT DO NOTHING;

-- Gastos Não Essenciais (regular small non-survival expenses)
INSERT INTO finance_categories (user_id, envelope_id, name, color, icon, budget_monthly, keyword_rules) VALUES
  (uid, env_res, 'Streaming',         '#EC4899', '🎬',  10.00, '["netflix","youtube","disney","spotify","deezer","apple tv","canal+","streaming"]'),
  (uid, env_res, 'Compras',           '#EC4899', '🛍️', 100.00, '["amazon","fnac","zara","h&m","primark","uniqlo","asos","vinted","leboncoin","compras","shopping"]'),
  (uid, env_res, 'Cuidados Pessoais', '#EC4899', '💆',  50.00, '["coiffeur","salon","esthetique","spa","parfum","sephora","cuidados","corte de cabelo"]'),
  (uid, env_res, 'Presentes',         '#EC4899', '🎁',  50.00, '["cadeau","presente","anniversaire","noel","fleurs"]'),
  (uid, env_res, 'Educação',          '#EC4899', '📚', 100.00, '["udemy","coursera","livres","livre","formation","cours","educacao","escola"]')
ON CONFLICT DO NOTHING;

-- Torrar (leisure & fun — guilt-free spending)
INSERT INTO finance_categories (user_id, envelope_id, name, color, icon, budget_monthly, keyword_rules) VALUES
  (uid, env_liv, 'Bares e Restaurantes', '#A855F7', '🍽️', 200.00, '["restaurant","brasserie","bistrot","bar","cafe","deliveroo","ubereats","just eat","rappi","ifood"]'),
  (uid, env_liv, 'Balada',               '#A855F7', '🎶', 100.00, '["club","boite","concert","show","festival","billeterie","fnac spectacles"]'),
  (uid, env_liv, 'Viagem',               '#A855F7', '✈️', 400.00, '["airfrance","easyjet","ryanair","airbnb","booking","hotels","voyage","viagem","smiles","tudo azul","latam","gol"]'),
  (uid, env_liv, 'Lazer',                '#A855F7', '🎭', 100.00, '["cinema","theatre","musee","parc","loisir","lazer","piscine","sport","salle de sport","academia"]')
ON CONFLICT DO NOTHING;
